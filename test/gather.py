...
lib_path = os.path.join(os.path.dirname(
    os.path.abspath(__file__)), '.././build/lib/libmy_library.so')
...
def test(inputShape, indexShape, axis, test_dtype, device):
    ...
    input_ptr = ctypes.cast(inputTensor.data_ptr(),
                            ctypes.POINTER(ctypes.c_void_p))
    index_ptr = ctypes.cast(indexTensor.data_ptr(),
                            ctypes.POINTER(ctypes.c_void_p))
...
parser = argparse.ArgumentParser(
    description="Test Gather on different devices.")
parser.add_argument('--device', choices=['cpu', 'cuda', 'mlu'],
                    required=True, help="Device to run the tests on.")
...
test_cases = [
    ...
    ((1024, 512, 32, 4), (128, 2, 2), 1, torch.float32, "cuda"),
